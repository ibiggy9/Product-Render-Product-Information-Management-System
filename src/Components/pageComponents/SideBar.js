...
export default function SideBar({brands, options}) {
    const keyActions = [ 
        {action: "View catalog & Update Content", link:"/catalog"},
        {action: "Add new products", link: "/addProduct"},
          ...
          <NavLink to='/'>
              <nav className="px-4 pt-2.5 border-b ">
                <div id="logo" className="flex items-center">
                  ...
                  <motion.div whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.8 }}  ><span className="mb-3 ml-3 self-center text-xl font-semibold text-white">SupplierRender</span></motion.div>
                </div> 
             </nav>  
          </NavLink>
          </div>
